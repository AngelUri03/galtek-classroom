...
$projectPath = Resolve-FullPath (Join-Path $repositoryRoot 'agent\src\GaltekClassroom.Agent.Service\GaltekClassroom.Agent.Service.csproj')
...
if (-not (Test-Path -LiteralPath $projectPath -PathType Leaf)) {
    throw "Agent Service project was not found: $projectPath"
}
...
$serviceExecutablePath = Join-Path $publishOutputPath $ServiceExecutableName
if (-not (Test-Path -LiteralPath $serviceExecutablePath -PathType Leaf)) {
    throw "Publish did not produce expected executable: $serviceExecutablePath"
}

Write-Host "Agent Service artifact published."
