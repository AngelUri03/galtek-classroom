...
Prompt 19G2 completa el tramo local Agent Service -> Credential Provider V2 -> credential serialization soportada por Windows para una activation ya existente, sin agregar todavia la operacion remota `LOGON_MANAGED_ACCOUNT`. El provider nativo C++ sigue siendo aditivo, soporta solo `CPUS_LOGON`, no implementa `ICredentialProviderFilter`, no oculta Password/PIN/Windows Hello/otros providers y no abre red. Sin activation/identity valida enumera 0 credentials Galtek; con activation vigente, binding local y SID `SidTypeUser` resoluble por el Service, enumera 1 credential Galtek sin campo password visible/editable.

El bridge local dedicado `GaltekClassroom.CredentialProvider.v1` conserva el caller validation de 19G1: PID real del named pipe, proceso real, image path canonico `%SystemRoot%\System32\LogonUI.exe`, sesion interactiva y token real `S-1-5-18`, sin confiar en PID/SID/username/process enviados por payload. 19G2 agrega `GET_PENDING_ACTIVATION_IDENTITY`, respuesta JSON no secreta con `activationId`, `accountId`, `userSid`, `domain` y `username`, todos derivados por el Agent Service desde `managed-windows-accounts.json` + `LookupAccountSidW`/resolver soportado y no desde `accountReference` ni Master. La primera identity exitosa fija en memoria el `expectedWindowsSid` de la activation.

19G2 agrega `ACQUIRE_PENDING_CREDENTIAL(activationId)` como unica operacion secreta local. El request no acepta accountId, username, domain, SID, password, credentialId, token, sessionId, PID, proceso, comando ni payload arbitrario. Antes de revelar secreto, el Service relee el binding actual y exige que `windowsSid` siga coincidiendo con el SID fijado por identity; si hubo rebind, falla cerrado. La activation se marca `CONSUMED` de forma atomica antes de llamar `ManagedWindowsCredentialStore.AcquireForWindowsSidAsync(...)`; un segundo acquire no vuelve a llamar DPAPI. La respuesta de acquire es binaria, versionada y acotada dentro del framing big-endian vigente, con magic/version/status/activationId/passwordByteLength/password UTF-16LE; no usa JSON, Base64, hexadecimal, XML ni Protobuf para password.

`GetSerialization` adquiere la password como maximo una vez, usa buffers mutables RAII, llama `CredProtectW` para proteccion Windows previa al packing, construye `KERB_INTERACTIVE_UNLOCK_LOGON` con `MessageType = KerbInteractiveLogon`, resuelve el paquete `Negotiate` mediante LSA lookup soportado y devuelve `CPGSR_RETURN_CREDENTIAL_FINISHED` solo con buffer completo y CLSID `{D1A77223-ACAE-4C53-8C52-4FE8B8357E82}`. Ante fallo despues de acquire no restaura la activation, no reintenta y devuelve `CPGSR_NO_CREDENTIAL_NOT_FINISHED`. `GetUserSid` devuelve el SID obtenido del Service con ownership COM. La serialization entregada a LogonUI contiene material sensible; el provider transfiere ownership y no la loguea, cachea ni persiste.
