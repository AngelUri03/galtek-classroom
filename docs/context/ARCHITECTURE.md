...
Prompt 19G1 agrega la foundation de `Credential Provider V2` para el futuro `LOGON_MANAGED_ACCOUNT`, sin realizar login todavia. Se crea un proyecto nativo C++/Windows SDK en `agent/native/GaltekClassroom.CredentialProvider/` que implementa `ICredentialProvider`, `ICredentialProviderCredential` e `ICredentialProviderCredential2`, soporta solo `CPUS_LOGON`, no implementa `ICredentialProviderFilter`, no oculta Password/PIN/Windows Hello/otros providers y en 19G1 devuelve 0 credentials productivas. `GetSerialization` existe solo como superficie de interfaz y devuelve `CPGSR_NO_CREDENTIAL_NOT_FINISHED`, sin password, LSA ni `KERB_INTERACTIVE_UNLOCK_LOGON`.

Prompt 19G1 agrega tambien un bridge local Service -> Credential Provider mediante Named Pipe dedicado `GaltekClassroom.CredentialProvider.v1`, separado de Local IPC v1 y Session Command v1, con JSON UTF-8 versionado y framing big-endian acotado. El Agent Service es servidor y autoridad; el provider nativo es cliente local y no abre red. El pipe concede acceso solo a `LocalSystem`; el Service valida el caller con PID real del named pipe (`GetNamedPipeClientProcessId`), image path real canonico `%SystemRoot%\System32\LogonUI.exe`, sesion interactiva y token real `S-1-5-18`, sin confiar en PID/SID/username/process enviados por payload. La activation metadata es efimera, in-memory, una por Client, lazy-expiring, `PRIMARY`/`SECONDARY` solamente, sin password ni `ManagedWindowsCredentialStore.Acquire()`. Registro/desregistro queda solo en scripts lab-only y no se integra al installer productivo general.
