...
Prompt 19I1 no cambia semantica de LOGON/SWITCH, capabilities, secretos ni protocolo. Convierte el Credential Provider en componente instalable del Agent: publish produce artifact verificable, install registra machine-wide x64 solo el CLSID Galtek desde una version inmutable bajo Program Files, update hace side-by-side sin sobrescribir DLL cargada por LogonUI y uninstall elimina primero registration antes de cleanup best-effort. La validacion real de Windows logon/switch queda pendiente para 19I2 en PC descartable.

Prompt 19H2 agrega retry administrativo explicito y selectivo para `SWITCH_MANAGED_ACCOUNT` sobre la misma `BatchOperation` creada por 19H1. La maestra solicita `POST /api/operations/{operationId}/retry` con solo `targetDeviceIds`; el Master exige autorizacion local antes de leer storage, rechaza cualquier override de target/source/secrets/force/allFailed y obtiene `targetAccountId` solo del payload durable original `schemaVersion = 1`.

El retry 19H2 solo acepta targets que pertenecen al batch original y cuyo resultado actual esta `FAILED` con `errorCode` no nulo y retryable. `SUCCESS`, `NO_CHANGE`, `PENDING` y `OPERATION_RESULT_UNKNOWN` de switch nunca se reintentan. La seleccion se reclama de forma transaccional antes del primer trabajo remoto pasando `FAILED -> PENDING` con `attempt + 1`; un conflicto concurrente aborta todo sin fanout. Luego se ejecutan preflight tecnico fresco, snapshot fresco `GET_WINDOWS_SESSION_STATE` con operationId remoto nuevo y, si corresponde mutation, la misma primitive `SWITCH_MANAGED_ACCOUNT(targetAccountId original)` con otro operationId remoto nuevo. No hay retry automatico, scheduler ni recuperacion automatica de targets que queden `PENDING` tras power loss.

Prompt 19H1 agrega el batch Master productivo `SWITCH_MANAGED_ACCOUNT` para Devices seleccionados explicitamente. La maestra solicita una sola accion con `targetAccountId = PRIMARY|SECONDARY` y `targetDeviceIds`; el Master crea una unica `BatchOperation`, la persiste antes del primer snapshot remoto, consulta `GET_WINDOWS_SESSION_STATE` por target con operationId independiente y usa `ManagedAccountSwitchPlanner` solo sobre estado real de sesion. `PRIMARY_ACTIVE`/`SECONDARY_ACTIVE` que ya coinciden producen `NO_CHANGE`; `NO_SESSION` planifica `LOGON`; opposite managed activo planifica `SWITCH`; `OTHER_SESSION_ACTIVE` bloquea como `WINDOWS_SESSION_CHANGED`; `UNKNOWN` bloquea como `WINDOWS_SESSION_UNKNOWN`.

Aunque el plan pueda decir `LOGON` o `SWITCH`, la mutation remota enviada por el Master siempre es `SWITCH_MANAGED_ACCOUNT(target)`, nunca una cadena Master-side `LOGOFF_WINDOWS_SESSION + LOGON_MANAGED_ACCOUNT`. El Agent conserva la autoridad ante races, bindings, credenciales, Credential Provider y efectos parciales. `NO_CHANGE` cuenta como exito final, se persiste, no es retryable y no envia mutation. El Master no fabrica `accountConfigured` ni `credentialConfigured`: errores como `ACCOUNT_NOT_CONFIGURED`, `MANAGED_CREDENTIAL_NOT_CONFIGURED` o `CREDENTIAL_PROVIDER_UNAVAILABLE` se preservan desde el Agent. No hay retry automatico.

Prompt 19G4 completa `SWITCH_MANAGED_ACCOUNT(targetAccountId)` como primitive remota individual Master gateway -> Agent. El Master envia solo `PRIMARY` o `SECONDARY`; no envia source account, password, username, SID, sessionId, `accountReference`, credential/vault ids, force, timeout configurable ni comandos. El Agent deriva source exclusivamente desde `WindowsSessionState` local: target ya activo es `SUCCESS` idempotente; `NO_SESSION` reutiliza `LOGON_MANAGED_ACCOUNT`; opposite managed activo ejecuta preflight target antes de cerrar source, revalidacion source, `LOGOFF source`, espera local acotada a `NO_SESSION` y luego `LOGON target`; `OTHER_SESSION_ACTIVE` nunca se toca y devuelve `WINDOWS_SESSION_CHANGED`; `UNKNOWN` devuelve `WINDOWS_SESSION_UNKNOWN`.

SWITCH es una operacion compuesta con posible efecto parcial despues de que Windows acepta `WTSLogoffSession(..., FALSE)`. WTS accepted no equivale a source terminada, por lo que el Agent no inicia target logon hasta confirmar `NO_SESSION`; si no se confirma antes del deadline devuelve `WINDOWS_SWITCH_NOT_CONFIRMED`. Al cerrar 19G4 no habia rollback automatico a source, retry automatico, reconciliation/journal nuevo, endpoint HTTP, BatchOperation, fanout, planner, UI ni cambios C++ del Credential Provider; 19H1 agrega el batch/endpoint Master sin cambiar la primitive Agent-side.

Prompt 19G3 completa `LOGON_MANAGED_ACCOUNT` remoto end-to-end para un Client individual usando Credential Provider V2 y el flujo normal Winlogon/LSA. El Master envia solo `accountId` logico `PRIMARY`/`SECONDARY` por `MasterRemoteOperationGateway.logonManagedAccount(...)`; no envia password, username, SID, domain, sessionId, credentialId, vault token, comando ni payload generico. El Agent exige licencia comercial activa, trust/dispositivo/operacion validos, preflight de consola fisica, binding local, SID `SidTypeUser`, credencial DPAPI usable y revalidacion inmediata de `NO_SESSION` antes de crear una activation remota efimera. El provider es notificado por `WAIT_FOR_ACTIVATION_CHANGE`, auto-submit ocurre una vez, `GetSerialization` entrega `KERB_INTERACTIVE_UNLOCK_LOGON` y `REPORT_LOGON_RESULT` convierte `STATUS_SUCCESS` en `SUCCESS`; rechazos/fallos locales/timeout quedan como errores estructurados. Galtek sigue sin endpoint HTTP, BatchOperation, planner, UI, switch, Credential Provider Filter ni ocultamiento de providers estandar.

Prompt 07 define el dominio funcional, modelos puros y planners de preflight. Prompt 08 persiste ese dominio en SQLite local para el Master. Prompt 10 expone la primera API administrativa protegida sobre SQLite con bootstrap, snapshot, CRUD escolar, batches de alumnos y assignments de metadata. Prompt 9.6 formaliza cuentas Windows administradas futuras en Clients (`PRIMARY`/`SECONDARY`) y cambio masivo de sesion como dominio puro. Prompt 14.2 fija el modelo operativo real del aula, readiness progresiva, workspace canonico Master/local working copy Client, prioridades y modos de proyeccion como dominio puro. Prompt 14.4 fija resiliencia ante apagones, startup rapido, boot storm y semantica de recovery sin implementar filesystem real, browser automation, UI, login/logoff Windows, USB, captura ni proyeccion. Prompt 15A agrega `SHUTDOWN` y `RESTART` productivos en el Agent sobre el framework seguro existente. Prompt 15B agrega dispatch batch desde Master para esas dos operaciones. Prompt 15C agrega reconciliacion segura de resultados inciertos sin UI, retry automatico ni nuevas operaciones Windows. Prompt 16A agrega el canal local seguro Service -> Session para acciones interactivas futuras, Prompt 16B implementa `OPEN_URL` productivo Agent-side sin endpoint batch Master, sin bloqueo de URLs/descargas y sin UI, Prompt 16C agrega en el Master el modelo persistente de politicas administrativas de navegacion, Prompt 16D agrega enforcement Agent-side para Chrome/Edge usando `URLBlocklist`/`URLAllowlist` en el usuario interactivo real, Prompt 16E1 agrega en el Master el modelo persistente de politicas de descarga de navegador, Prompt 16E2A prepara el contrato tipado y compilador C# puro de descargas, Prompt 16E2B agrega enforcement Agent-side real de descargas mediante `DownloadRestrictions`, Prompt 16F1 agrega dispatch batch Master para aplicar policies de navegacion y descarga persistidas, Prompt 16F2 agrega dispatch batch Master para `OPEN_URL`, Prompt 17A agrega el catalogo local `application-bindings.json`, Prompt 17B implementa `OPEN_APPLICATION(applicationId)` productivo Agent-side, Prompt 17C agrega dispatch batch Master para `OPEN_APPLICATION`, Prompt 18A implementa `LOCK_INPUT`/`UNLOCK_INPUT` productivo Agent-side mediante Session Agent y `BlockInput`, Prompt 18B1 agrega autorizacion local read-only de proposito unico para `UNLOCK_INPUT` recovery desde Master sin exigir Commercial License local activa, Prompt 18B2 agrega dispatch batch Master para `LOCK_INPUT`/`UNLOCK_INPUT`, Prompt 19A agrega el nucleo interno de Credential Vault local cifrado para passwords Windows/Google escolares de la profesora sin UI ni endpoints, Prompt 19B agrega el binding local Client `PRIMARY`/`SECONDARY` -> Windows SID sin passwords ni sesion Windows real, Prompt 19C implementa `GET_WINDOWS_SESSION_STATE` Agent-side como snapshot read-only/on-demand de consola fisica mapeado por SID, Prompt 19D agrega el Client secure credential store DPAPI para passwords Windows de `PRIMARY`/`SECONDARY`, Prompt 19E1 agrega provisioning remoto seguro `PROVISION_MANAGED_CREDENTIAL` sobre gRPC/mTLS sin HTTP, BatchOperation, Credential Vault bridge, login ni switch, Prompt 19E2 agrega el bridge interno Master Credential Vault -> gateway para un solo Device sin endpoint HTTP ni BatchOperation, Prompt 19F implementa `LOGOFF_WINDOWS_SESSION` Agent-side seguro para cerrar solo la sesion de consola fisica cuyo SID coincide con el managed account esperado, Prompt 19G1/19G2/19G3 completan Credential Provider V2 y logon remoto individual, Prompt 19G4 completa switch remoto individual Agent-side, Prompt 19H1 agrega batch/endpoint Master para switch de Devices explicitos y Prompt 19H2 agrega retry administrativo selectivo del mismo batch.
...
- Desde Prompt 19D, `credentialConfigured` puede ser `true` solo para una credencial DPAPI usable del Client ligada al `installationId`, `accountId` y mismo `windowsSid` del binding vigente.
- `READY` requiere binding configurado, SID resoluble como `SidTypeUser` y credencial usable ligada al mismo SID. No significa que la password haya sido validada con un logon real.
- Si falta credencial, el estado es `CREDENTIAL_NOT_CONFIGURED`; si la cuenta ligada desaparecio, `ACCOUNT_NOT_FOUND`.
...
- La credencial real usada por login/provisioning futuro pertenece al Agent Service del Client y se almacena en `managed-windows-credentials.dat`, separado del binding SID.
- El secreto se protege con Windows DPAPI bajo LocalSystem, scope de usuario actual, sin LocalMachine ni fallback plaintext.
- La credencial incluye el SID del binding dentro del payload protegido; rebind a otro SID invalida logicamente la credencial anterior sin borrado automatico.
- El Client no ofrece reveal/export/dump de passwords; la consulta humana pertenece al Credential Vault del Master.
...
- El login administrado productivo usa Credential Provider V2 y el switch productivo reutiliza LOGOFF + espera `NO_SESSION` + LOGON; no usar SendKeys, scripts, PowerShell, `cmd` ni autologon inseguro para iniciar o cambiar sesion.
- Siempre debe conservarse una via estandar de acceso/recovery de Windows.

Desde Prompt 19E1, `PROVISION_MANAGED_CREDENTIAL` establece la copia segura que Galtek Client conserva para un slot `PRIMARY` o `SECONDARY`: crea la entry si falta o la reemplaza si existe. El Master envia solo `accountId` tipado y `password_utf16le` como bytes UTF-16LE; el Client valida binding/SID local, protege inmediatamente por DPAPI y persiste `managed-windows-credentials.dat`. `SUCCESS` no valida la password contra Windows, no cambia la password real de Windows, no crea login, no cambia SID/binding/username/perfil/sesion y no implica cuenta desbloqueada. Si falta `OperationResult`, el resultado sigue siendo `OPERATION_RESULT_UNKNOWN` sin retry automatico.

El dedupe de `PROVISION_MANAGED_CREDENTIAL` es secret-safe: mismo `operationId + accountId` devuelve el resultado original sin reaplicar la password, incluso con bytes distintos en el duplicado; mismo `operationId` con otro accountId produce conflicto. No se persisten ni cachean password, hash, fingerprint de password ni request protobuf secreto completo.

Prompt 19E1 no implementa Credential Vault bridge, Credential Provider, login/logoff real, switch real, IPC credential op, endpoint HTTP Master, BatchOperation, UI ni browser policy enforcement por `PRIMARY`/`SECONDARY`.

Desde Prompt 19E2, el Master Backend puede provisionar internamente una credencial ya almacenada en Credential Vault hacia un solo Client mediante `ManagedCredentialProvisioningBridge`. El caller interno entrega `vaultSessionToken`, `credentialId`, `deviceId`, `operationId` y `accountId` `PRIMARY`/`SECONDARY`; el bridge usa `MasterAccessGuard`, exige sesion de vault vigente, acepta solo entries `WINDOWS_ACCOUNT`, rechaza `GOOGLE_ACCOUNT` como `CREDENTIAL_NOT_PROVISIONABLE`, obtiene la password solo desde el vault, la codifica a UTF-16LE temporal sin BOM/NUL y limpia el `byte[]` controlado. `credentialId` y vault token nunca salen del Master; el resultado tipado del gateway se propaga, incluido `OPERATION_RESULT_UNKNOWN`, sin retry automatico. 19E2 no agrega endpoint HTTP, UI, BatchOperation, fanout, SQLite writes, login/logoff/switch ni cambios Client.
...
`WindowsSessionState` representa el estado observado de sesion Windows en un Client:
...
Desde Prompt 19C, `GET_WINDOWS_SESSION_STATE` observa un snapshot de la consola fisica actual. La autoridad inicial es `WTSGetActiveConsoleSessionId()`. Galtek no elige sesion por `explorer.exe`, procesos, foreground window, username, WMI, Registry, perfiles en `C:\Users`, RDP ni presencia del Session Agent.

La identidad se obtiene del SID del `TokenUser` del token real de la sesion de consola mediante `WTSQueryUserToken` y `GetTokenInformation(TokenUser)`. El Service debe correr como LocalSystem y habilitar `SeTcbPrivilege` de forma acotada. El token se usa solo para leer el SID y se cierra siempre.

`WTSUserName` puede consultarse solo como senal auxiliar para distinguir ausencia de login: username vacio significa `NO_SESSION`. Ese username nunca mapea `PRIMARY`/`SECONDARY`, nunca se persiste, nunca se envia al Master y nunca se compara con `accountReference`.

Reglas de mapping:

- sin usuario logueado en consola fisica -> `NO_SESSION`;
- SID activo igual a `PRIMARY.windowsSid` -> `PRIMARY_ACTIVE`;
- SID activo igual a `SECONDARY.windowsSid` -> `SECONDARY_ACTIVE`;
- usuario real con SID distinto, incluso sin bindings configurados -> `OTHER_SESSION_ACTIVE`;
- `WTSGetActiveConsoleSessionId() == 0xFFFFFFFF`, Session 0 o incertidumbre confiable -> `UNKNOWN`;
- falla tecnica para obtener observacion confiable -> error operacional `WINDOWS_SESSION_UNKNOWN`;
- `managed-windows-accounts.json` corrupto/schema/mismatch -> `MANAGED_ACCOUNT_BINDINGS_INVALID`.

Una sesion bloqueada conserva la cuenta logueada y se clasifica igual que desbloqueada. Sesiones disconnected o RDP historicas no reemplazan la consola fisica. `GET_WINDOWS_SESSION_STATE` no depende de `SESSION_AGENT_AVAILABLE`, no escribe archivos, no hace polling y no aparece en heartbeat.
...
Desde Prompt 19F, `LOGOFF_WINDOWS_SESSION(accountId)` es una operacion destructiva explicita del Agent Service. El unico parametro funcional es `accountId` tipado `PRIMARY` o `SECONDARY`; `UNSPECIFIED` se rechaza y no se transportan username, domain, SID, `accountReference`, password, sessionId, force, timeout, command ni payload libre.

La semantica es expected-account: "cerrar la sesion fisica actual solo si todavia pertenece al managed account indicado". El Agent exige binding local vigente para ese slot; binding ausente produce `ACCOUNT_NOT_CONFIGURED` y binding corrupto/schema/mismatch produce `MANAGED_ACCOUNT_BINDINGS_INVALID`. No se consulta `managed-windows-credentials.dat`, DPAPI ni Credential Vault.

El Agent observa la consola fisica mediante `WTSGetActiveConsoleSessionId()`, obtiene el SID real con `WTSQueryUserToken` + `GetTokenInformation(TokenUser)` y lo compara contra `managed-windows-accounts.json`. Si no hay usuario logueado, `SUCCESS` es idempotente porque la sesion esperada ya esta ausente. Si hay otro managed account, un administrador u otro usuario, devuelve `WINDOWS_SESSION_CHANGED` y no toca esa sesion. Si la consola no es confiable, devuelve `WINDOWS_SESSION_UNKNOWN`.

Antes del logoff destructivo, el Agent repite la observacion y exige mismo `sessionId` y mismo SID esperado. Solo entonces llama `WTSLogoffSession(WTS_CURRENT_SERVER_HANDLE, sessionId, FALSE)`. `SUCCESS` significa que Windows acepto la solicitud asincrona, no que el cierre ya concluyo; una consulta futura explicita `GET_WINDOWS_SESSION_STATE` puede observar el resultado. No hay polling posterior, retry automatico, status heartbeat ni reconciliacion 19F.

`LOGOFF_WINDOWS_SESSION` puede cerrar aplicaciones y perder trabajo no guardado segun Windows/aplicaciones; no se ejecuta automaticamente por assignment, startup ni recovery. La UI futura debera presentar una accion administrativa explicita y advertencia adecuada. 19F no agrega `force=true`.

## ManagedAccountSwitchPlanner

`ManagedAccountSwitchPlanner` planifica `SWITCH_MANAGED_ACCOUNT(targetAccountType)` sobre snapshots remotos reales por Device. No inicia sesion, no cierra sesion, no toca Windows, no consulta Vault y no conoce passwords.
...
- Otra sesion no administrada: `BLOCKED` con `WINDOWS_SESSION_CHANGED`.
- Sesion desconocida: `BLOCKED` con `WINDOWS_SESSION_UNKNOWN`.
- Cuenta no configurada, credencial no configurada o Credential Provider no disponible no se fabrican en el Master; si aplican, los devuelve el Agent durante `SWITCH_MANAGED_ACCOUNT`.
...
PC05 OTHER      -> BLOCKED/WINDOWS_SESSION_CHANGED
```
...
Ambos aceptan solo `targetDeviceIds` explicitos, no aceptan `type` ni parametros funcionales. `lock` ejecuta `MasterAccessGuard` antes de cualquier lectura escolar; `unlock` ejecuta `MasterUnlockAccessGuard` antes de cualquier lectura escolar. El bypass de licencia local Master queda aislado al flujo `UNLOCK_INPUT` y no autoriza otras acciones.

El preflight Master exige Device del aula, binding vigente, Network Identity esperada, trust `PAIRED`, no `REVOKED`, conexion gRPC/mTLS `ONLINE` y capability `INPUT_CONTROL_V1`. No exige `SESSION_AGENT_AVAILABLE`, no consulta Student/Group/DeviceAssignment y no evalua la Commercial License del Client.

Cada request crea una sola `BatchOperation` antes del fanout, con `operationType = LOCK_INPUT` o `UNLOCK_INPUT`, payload minimo `schemaVersion = 1`, targets listos `PENDING` y fallos de preflight `FAILED`. El gateway envia la operacion tipada sin parametros funcionales y usa el mismo `operationId` por batch para todos los Agents. `OperationAccepted` no es exito; solo `OperationResult SUCCESS` marca `SUCCESS`.

El estado de lock es efimero. El batch registra resultado de la accion, no current state durable. `CTRL+ALT+DEL` puede liberar el lock y el Master no lo detecta, no hace re-lock, no agrega status endpoint, no reconcilia y no reintenta automaticamente. Si `LOCK_INPUT` o `UNLOCK_INPUT` quedan sin `OperationResult`, el target queda `OPERATION_RESULT_UNKNOWN`; la recuperacion correcta ante lock incierto es una nueva request explicita `UNLOCK_INPUT`.
...
Prompt 08 persiste `BatchOperation` y `BatchTargetResult` con `operationId`, targets, estados, errores, mensaje operacional, `attempt` y payload JSON versionado. El retry se calcula solo sobre targets fallidos cuyo `ErrorCode` sea retryable, con exclusiones especificas por operacion cuando el efecto no sea seguro.
...
- `POST /api/operations/{operationId}/retry` reintenta explicitamente targets seleccionados de operaciones `SWITCH_MANAGED_ACCOUNT`, sin crear batch nuevo.
...
La retryability productiva base vive en `ErrorCode.retryable()`. Cada operacion puede imponer restricciones mas fuertes que esa flag segun su semantica y efectos posibles. Una operacion nunca debe volverse retryable solo porque un error aparecia en una lista conceptual historica, por texto de categoria o por parecer temporal.

Ejemplos de errores que normalmente requieren intervencion o una regla especifica:
...
`LOGOFF_WINDOWS_SESSION` no debe tener retry automatico aunque parezca idempotente: si el Master envio la operacion y no recibe `OperationResult`, un retry tardio podria encontrar una nueva sesion del mismo `accountId` y cerrarla. El resultado debe conservarse como `OPERATION_RESULT_UNKNOWN` hasta una accion o consulta explicita futura.

`SWITCH_MANAGED_ACCOUNT` no tiene retry automatico ni reconciliation productiva. El retry administrativo explicito de 19H2 solo reintenta targets seleccionados del batch original cuando el target actual es `FAILED` y su `errorCode` productivo es retryable. `SUCCESS`, `NO_CHANGE` y `PENDING` nunca se reintentan. `OPERATION_RESULT_UNKNOWN` de SWITCH nunca se reintenta, aunque una metadata generica futura cambiara. El retry conserva `operationId`, `createdAtUtc`, `requestedBy`, `targetCount` y payload; reclama transaccionalmente los targets con `attempt + 1`; usa preflight tecnico y snapshot fresco; y nunca acepta cambiar `targetAccountId` desde el body de retry.

Errores de autenticacion/logon no deben declararse retryable por texto, por categoria generica ni por listas documentales antiguas; se respeta la metadata productiva real y las restricciones de la operacion concreta. Operaciones con semantica especial, como power reconciliation, conservan sus propias reglas explicitas.
