...
Prompt 19G2 completa el tramo local seguro para convertir una activation ya existente en una credential Galtek productiva mediante Credential Provider V2. El provider nativo C++/Windows SDK mantiene CLSID `{D1A77223-ACAE-4C53-8C52-4FE8B8357E82}`, soporta solo `CPUS_LOGON`, no implementa `ICredentialProviderFilter`, no oculta Password/PIN/Windows Hello/otros providers, no abre red y enumera 0 credentials Galtek si no hay activation + identity valida. Con activation vigente, binding local y SID `SidTypeUser` resoluble por el Agent Service, enumera 1 credential Galtek sin campo password visible/editable.

El bridge local dedicado `GaltekClassroom.CredentialProvider.v1` conserva el caller validation de 19G1: PID real del pipe, proceso real, ruta canonica `%SystemRoot%\System32\LogonUI.exe`, sesion interactiva y token real `S-1-5-18`, sin aceptar hints de autorizacion por payload. `GET_PENDING_ACTIVATION_IDENTITY` devuelve JSON no secreto con `activationId`, `accountId`, `userSid`, `domain` y `username`, derivados por el Service desde Installation Identity, `managed-windows-accounts.json`, SID valido y resolver Windows `SidTypeUser`; no usa `accountReference` como autoridad ni Master. La primera identity exitosa fija en memoria el `expectedWindowsSid`.

`ACQUIRE_PENDING_CREDENTIAL(activationId)` es one-time y secreto: no acepta accountId, SID, username, domain, password, credentialId, token, PID, proceso ni payload arbitrario. Antes de revelar, el Service relee el binding vigente y exige que siga coincidiendo con el SID fijado; luego marca la activation `CONSUMED` atomicamente antes de llamar `ManagedWindowsCredentialStore.AcquireForWindowsSidAsync(...)`. El segundo acquire no llama DPAPI. La respuesta secreta es binaria, versionada y acotada dentro del framing big-endian, con magic/version/status/activationId/passwordByteLength/password UTF-16LE; no usa JSON, Base64, hexadecimal, XML ni Protobuf para password.

`GetUserSid` devuelve el SID obtenido desde el Service con ownership COM. `GetSerialization` realiza como maximo un acquire por credential, recibe password en buffer RAII mutable, usa `CredProtectW`, construye `KERB_INTERACTIVE_UNLOCK_LOGON` con `MessageType = KerbInteractiveLogon`, resuelve `Negotiate` via LSA lookup soportado y devuelve `CPGSR_RETURN_CREDENTIAL_FINISHED` solo cuando el buffer completo queda armado. Ante fallo despues de acquire no restaura activation, no reintenta y devuelve `CPGSR_NO_CREDENTIAL_NOT_FINISHED`. Los buffers propios de secreto y response se limpian con zeroization; la serialization entregada a LogonUI transfiere ownership y no puede limpiarse inmediatamente por el provider.
...
Fase 19G2 deja completo el tramo local one-time Service -> Credential Provider -> Windows serialization para una activation ya existente. El siguiente paso recomendado, 19G3, es agregar `LOGON_MANAGED_ACCOUNT` remoto, capability, `OperationRequest` tipado, creation productiva de activation, notification event-driven al provider, `CredentialsChanged` y resultado operacional seguro, sin mezclar todavia `SWITCH_MANAGED_ACCOUNT`.
