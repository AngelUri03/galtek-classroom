...
Discovery, screen streams, projection and most administrative commands remain planned features. `SHUTDOWN` and `RESTART` are the first productive Agent-side operations, exposed only through the typed remote operation framework. Uncertain power-control operations are reconciled by asking about the original `operationId`; the protocol does not automatically resend destructive requests.
