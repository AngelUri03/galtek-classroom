...
Discovery, screen streams, projection and most administrative commands remain planned features. Productive Agent-side operations currently include `SHUTDOWN`, `RESTART`, `OPEN_URL`, `APPLY_BROWSER_NAVIGATION_POLICY` and `APPLY_BROWSER_DOWNLOAD_POLICY`, exposed only through the typed remote operation framework. Uncertain power-control operations are reconciled by asking about the original `operationId`; the protocol does not automatically resend destructive requests.
